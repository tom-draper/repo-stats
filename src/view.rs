...
fn display_total(total: &Total, indent: i32) {
    ...
    println!("{}Files: {:>width$}", padding, format!("{}", total.files.to_formatted_string(&Locale::en)).yellow(), width=width);
    println!("{}Lines: {:>width$}", padding, format!("{}", total.lines.to_formatted_string(&Locale::en)).yellow(), width=width);
    println!("{}Characters: {:>width$}", padding, format!("{}", total.chars.to_formatted_string(&Locale::en)).yellow(), width=width-5);
    println!("{}Average line length: {:>width$}", padding, format!("{:.1}", total.avg_line_len()).yellow(), width=width-14);
    println!("{}Whitespace: {:>width$}", padding, format!("{:.2}%", total.per_whitespace()).yellow(), width=width-5);
    println!("{}Memory: {:>width$}", padding,format!("{}",  total.memory).yellow(), width=width-1);
    println!("{}Carridge returns: {:>width$}", padding,format!("{}",  total.cr).yellow(), width=width-11);
}
